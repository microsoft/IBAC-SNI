...
class CrossingEnv(MiniGridEnv):
    ...
    """
    def __init__(self, size=9, num_crossings=None, min_num_crossings=None, max_num_crossings=None, obstacle_type=Lava, seed=None):

        if num_crossings is not None:
            assert min_num_crossings is None and max_num_crossings is None
        if min_num_crossings is not None:
            assert num_crossings is None and max_num_crossings is not None

        self.num_crossings = num_crossings
        self.min_num_crossings = min_num_crossings
        self.max_num_crossings = max_num_crossings
    ...
    def _gen_grid(self, width, height):
        ...
        if self.num_crossings is not None:
            rivers = rivers[:self.num_crossings]  # sample random rivers
        else:
            rivers = rivers[:self._rand_int(self.min_num_crossings, self.max_num_crossings+1)]
...
register(
    id='MiniGrid-SimpleCrossings-N3r-v0',
...
register(
    id='MiniGrid-SimpleCrossings-N2r-v0',
    entry_point='gym_minigrid.envs:CrossingEnv',
    reward_threshold=1000.0,
    kwargs = {
        'min_num_crossings': 0,
